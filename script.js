...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentMealType = null; // NEW: To track which meal modal is for
    let dailyData = {
    ...
    // --- Event Listeners ---
    // Meal headers open the modal (for capturing/logging)
    ...
    // This ensures ALL modal close buttons work, including meal entry modal
    document.querySelectorAll('.modal-close-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const modalId = btn.getAttribute('data-modal-id');
            document.getElementById(modalId).classList.add('hidden');
            if (modalId === 'meal-entry-modal') {
                closeInputContainer(); // Clean up camera/results
            }
        });
    });

    // --- Functions ---
    // Populate the MEAL ENTRY MODAL
    function openInputContainer(mealType) {
        ...
        `;
        if (window.lucide?.createIcons) lucide.createIcons(); // re-init icons if available
        ...
        camBtn.addEventListener('click', (e) => {
            // FIX: use currentTarget (the button) not e.target (icon/text)
            toggleCamera(mealType, e.currentTarget);
        });
    }

    // Close and clean the input modal
    ...
    function handleFileUpload(event, mealType) {
        ...
        if (file) {
            ...
            // FIX: allow picking the same file again later
            event.target.value = '';
        }
    }
    ...
    async function startCamera(mealType) {
        ...
        try {
            ...
            // FIX: ensure metadata is ready and playback started before scanning
            await videoEl.play();
            ...
            // Clear any stale interval first (safety)
    ...
    function captureAndAnalyze(mealType) {
        ...
        const videoEl = mealEntryContent.querySelector('.video-feed');
        // FIX: guard against zero-size frames
        if (!videoEl || !videoEl.videoWidth || !videoEl.videoHeight) return;

        const context = canvas.getContext('2d');
        if (!context) return; // safety if canvas missing
    ...
    async function analyzeImage(imageBase64, mealType) {
        ...
        try {
            ...
            if (data.error) resultDisplay.innerHTML = `<p>${data.error}</p>`;
            else { stopCamera(); displayMultiResults(data, mealType); }
        } catch (err) {
    ...
    async function refineImage(correctionText, mealType) {
        ...
        try {
            ...
            const data = await response.json();
            if (data.error) resultDisplay.innerHTML = `<p>${data.error}</p>`;
            else displayMultiResults(data, mealType);
        } catch (err) {
    ...
    function displayMultiResults(items, mealType) {
        ...
        <div class="refine-section">
            <button class="refine-btn"><i data-lucide="wand-2"></i> Refine or Correct Results</button>
            <div class="refine-input-area hidden">
                <textarea placeholder="e.g., 'That's not chicken, it's paneer.'"></textarea>
                <button class="refine-submit-btn">Submit Correction</button>
            </div>
        </div>
        `;
        if (window.lucide?.createIcons) lucide.createIcons();
        ...
        const refineBtn = resultDisplay.querySelector('.refine-btn');
        const refineInputArea = resultDisplay.querySelector('.refine-input-area');
        refineBtn.addEventListener('click', () => {
            refineInputArea.classList.remove('hidden');
            refineBtn.classList.add('hidden');
        });

        resultDisplay.querySelector('.refine-submit-btn').addEventListener('click', () => {
            const correctionText = refineInputArea.querySelector('textarea').value;
            if (correctionText) refineImage(correctionText, mealType);
        });
    }
    ...
    function calculateFinalMacros(quantity, unit, originalData) {
        ...
        if (unit === 'pcs' && originalData.gramsPerPiece) {
            totalGrams = numQuantity * originalData.gramsPerPiece;
        } else if (unit === 'oz') {
            totalGrams = numQuantity * 28.35;
        }
        ...
        return {
            calories: Math.round((originalNutrients.calories || 0) * scale),
            protein: Math.round((originalNutrients.proteinGrams || 0) * scale),
            carbs: Math.round((originalNutrients.carbsGrams || 0) * scale),
            fat: Math.round((originalNutrients.fatGrams || 0) * scale)
        };
    }
    ...
    function renderAllMeals() {
        for (const mealType in dailyData.meals) {
            ...
            if (dailyData.meals[mealType].length === 0) {
                 listEl.innerHTML = `<p style="text-align:center; color: var(--subtle-text-color); padding: 1rem 0;">No items logged yet. Click above to add!</p>`;
            } else {
                listEl.innerHTML = '';
                 dailyData.meals[mealType].forEach(item => {
                    ...
                    <div class="meal-item-view">
                        ...
                        <div class="meal-item-actions">
                            ...
                            <button class="meal-item-delete" title="Delete"><i data-lucide="trash-2"></i></button>
                        </div>
                    </div>
        ...
        if (window.lucide?.createIcons) lucide.createIcons(); // Initialize icons in meal list
    }
    ...
    function updateItem(mealType, itemId, newQuantity, newUnit) {
        ...
        if (newUnit === 'pcs' && item.gramsPerPiece) {
            totalGrams = newQuantity * item.gramsPerPiece;
        } else if (newUnit === 'oz') {
            totalGrams = newQuantity * 28.35;
        }
    ...
    // 2. Add delete button to each meal item and handle deletion

    // When rendering meal items, add a delete button/icon
    function renderMealList(mealType, mealItems) {
        const mealList = document.getElementById(`${mealType}-list`);
        mealList.innerHTML = '';
        mealItems.forEach((item, idx) => {
            const li = document.createElement('li');
            // ...existing code for meal item display...
            li.textContent = `${item.name} - ${item.portion}g - ${item.calories} kcal`;
            ...
            deleteBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                // Remove meal from data and re-render
                mealItems.splice(idx, 1);
                renderMealList(mealType, mealItems);
                updateTotals();
                lucide.createIcons();
            });
    ...
    // Make sure to call renderMealList() whenever you update meals
    // For example, after adding, editing, or deleting a meal
